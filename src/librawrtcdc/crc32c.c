...
/* Check for SSE 4.2.  SSE 4.2 was first supported in Nehalem processors
   ...
   processors. */
#ifndef DISABLE_HW_CRC32C
#define HW_CRC32C_STATUS "enabled"
#define SSE42(have) \
    do { \
        uint32_t eax, ecx; \
        eax = 1; \
...
#else
#define HW_CRC32C_STATUS "disabled"
#endif
...
#ifdef DISABLE_HW_CRC32C
    return crc32c_sw(crc, buffer, length);
#else
    int sse42;
    SSE42(sse42);
    return sse42 ? crc32c_hw(crc, buffer, length) : crc32c_sw(crc, buffer, length);
#endif
}
...
void rawrtc_crc32c_init() {
    // Just initialise both versions to be on the safe side
